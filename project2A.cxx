...
GLuint SetupPhase345DataForRendering()
{
  ...
  GLuint indices_vbo;    // Index buffer object
  ...
  GLuint p3vao = 0;
  glGenVertexArrays(1, &p3vao);
  glBindVertexArray(p3vao);
...
const char *phase345VertexShader =
  ...
  "  float v_dirX = cameraloc[0] - position[0];\n"
  "  float v_dirY = cameraloc[1] - position[1];\n"
  "  float v_dirZ = cameraloc[2] - position[2];\n"
  "  float v_norm = sqrt(v_dirX*v_dirX + v_dirY*v_dirY + v_dirZ*v_dirZ);\n"
  "  v_dirX /= v_norm;\n"
  "  v_dirY /= v_norm;\n"
  "  v_dirZ /= v_norm;\n"
  "  float LdotN = lightdir[0]*vertex_normal[0] + lightdir[1]*vertex_normal[1] + lightdir[2]*vertex_normal[2];\n"
  "  float diffuse = max(0.0, LdotN);\n"
  "  float RX = 2*LdotN*vertex_normal[0] - lightdir[0];\n"
  "  float RY = 2*LdotN*vertex_normal[1] - lightdir[1];\n"
  "  float RZ = 2*LdotN*vertex_normal[2] - lightdir[2];\n"
  "  float RdotV = RX*v_dirX + RY*v_dirY + RZ*v_dirZ;\n"
  "  RdotV = max(0, RdotV);\n"
  "  float specular = pow(RdotV, lightcoeff[3]);\n"
  "  specular = max(0, specular);\n"
  "  if (specular > 1) { specular = 1; }\n"
  "  shading_amount = lightcoeff[0] + diffuse * lightcoeff[1] + specular*lightcoeff[2];\n"
#endif
...
const char *phase345FragmentShader =
  ...
  "  if (data >= 1 && data <= 4.5) {\n"
  "    float r = 0.25 + (.75 * ((data - 1) / 3.5));\n"
  "    float g = 0.25 + (.75 * ((data - 1) / 3.5));\n"
  "    float b = 1.0;\n"
  "    frag_color = vec4(r, g, b, 1.0);\n"
  "  }\n"
  "  if (data >= 4.5 && data <= 6) {\n"
  "    float r = 1.0;\n"
  "    float g = 1.0 - (.75 * ((data - 4.5) / 1.5));\n"
  "    float b = 1.0 - (.75 * ((data - 4.5) / 1.5));\n"
  "    frag_color = vec4(r, g, b, 1.0);\n"
  "  }\n"
#endif
  ...
  "  frag_color[0] *= shading_amount;\n"
  "  frag_color[1] *= shading_amount;\n"
  "  frag_color[2] *= shading_amount;\n"
#endif
